...
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-linux-free$' "free Linux firmware is explicit"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-linux-nonfree$' "the Debian non-free firmware baseline is explicit"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-misc-nonfree$' "miscellaneous device firmware is explicit"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-realtek$' "common Realtek NAS NIC firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-iwlwifi$' "Intel Wi-Fi firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-atheros$' "Atheros network firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-brcm80211$' "Broadcom Wi-Fi firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-mediatek$' "MediaTek network firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-amd-graphics$' "AMD graphics firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        firmware-intel-graphics$' "Intel graphics firmware is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        intel-microcode$' "Intel CPU microcode is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        amd64-microcode$' "AMD CPU microcode is installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        nvme-cli$' "NVMe health and namespace diagnostics are installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        pciutils$' "PCI hardware can be inventoried locally"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        ethtool$' "NIC driver and link diagnostics are installed"
require_pattern "$IMAGE_DIR/mkosi.conf" '^        lm-sensors$' "hardware sensor diagnostics are installed"
...
require_pattern "$REPO_ROOT/appliance/native_storage.py" 'storage\.volume\.btrfs\.scrub\.schedule\.v1' "native storage advertises the guarded Btrfs scrub schedule"
require_pattern "$REPO_ROOT/deploy/appliance/btrfs_scrub_schedule_runner.py" 'configured is not True or policy\.get\("enabled"\) is not True' "Btrfs scrub timer fails closed without an explicit enabled policy"
...
require_pattern "$REPO_ROOT/appliance/native_storage.py" 'shared-folder\.snapshot\.lock\.v1' "native storage advertises approval-bound snapshot locks"
...
require_pattern "$REPO_ROOT/appliance/native_storage_routes.py" 'omv\.btrfs-snapshot\.lock' "snapshot locks require a dedicated approval action"
require_pattern "$REPO_ROOT/appliance/btrfs_snapshot_lock_policy.py" 'metadata\.st_mode & 0o077' "snapshot lock registry rejects group or world access"
...
require_pattern "$REPO_ROOT/deploy/appliance/btrfs_snapshot_schedule_runner.py" 'item\.get\("locked"\) is not True' "snapshot retention never prunes locked snapshots"
require_pattern "$REPO_ROOT/appliance/btrfs_snapshot_schedule_policy.py" 'echo\.btrfs-snapshot-schedule-desired\.v2' "snapshot retention publishes the versioned age policy"
require_pattern "$REPO_ROOT/deploy/appliance/btrfs_snapshot_schedule_runner.py" 'mode == "days"' "snapshot retention supports bounded day windows"
require_pattern "$REPO_ROOT/deploy/appliance/btrfs_snapshot_schedule_runner.py" 'mode == "months"' "snapshot retention supports bounded month windows"
...
require_pattern "$REPO_ROOT/deploy/provision/base/provision-lib.sh" 'systemctl enable --now echo-disk-idle\.service' "fresh appliances persist the HDD idle policy across boot"
require_pattern "$REPO_ROOT/deploy/provision/host_migration.py" '^SERVICES = \("echo-disk-idle\.service",\)$' "upgraded appliances enable the HDD idle boot service"
