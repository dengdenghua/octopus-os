"""Approval-bound policy for monthly Echo-managed Btrfs RAID1 scrubs."""
...
SCHEMA_VERSION = 1
DESIRED_SCHEMA = "echo.btrfs-scrub-schedule-desired.v1"
POLICY_PATH = Path("/etc/echo-os/btrfs-scrub-schedule.json")
TIMER_PATH = Path("/etc/systemd/system/echo-btrfs-scrub.timer")
SCHEDULE = "first Sunday of each month after 01:45 local time, randomized within 24 hours"
MAX_FILE_BYTES = 4096
MAX_TIMER_BYTES = 16 * 1024
_POLICY_LOCK = threading.RLock()


class BtrfsScrubSchedulePolicyError(ValueError):
    """A Btrfs scrub schedule policy or desired state is invalid."""
...
def validate_policy(value: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    if set(value) != {"schemaVersion", "enabled"}:
        raise BtrfsScrubSchedulePolicyError("Btrfs scrub policy has an unexpected schema")
    if value["schemaVersion"] != SCHEMA_VERSION or not isinstance(value["enabled"], bool):
        raise BtrfsScrubSchedulePolicyError("Btrfs scrub policy has invalid values")
...
def _safe_read(path: Path, *, trusted_uid: int) -> tuple[bool, bytes | None]:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
        raise OSError("Btrfs scrub policy is not a regular file")
    if metadata.st_uid != trusted_uid or (os.name == "posix" and metadata.st_mode & 0o022):
        raise OSError("Btrfs scrub policy has unsafe ownership or mode")
    if metadata.st_size > MAX_FILE_BYTES:
        raise OSError("Btrfs scrub policy exceeds the safety limit")
...
def read_policy(path: Path = POLICY_PATH, *, trusted_uid: int = 0) -> tuple[bool, dict[str, Any]]:
    ...
    try:
        ...
        raise OSError("Btrfs scrub policy is invalid JSON") from exc
    if not isinstance(value, dict):
        raise OSError("Btrfs scrub policy is not a JSON object")
    try:
        return exists, validate_policy(value)
    except BtrfsScrubSchedulePolicyError as exc:
...
def _desired(value: Mapping[str, Any]) -> dict[str, Any]:
    if set(value) != {"schema", "enabled"}:
        raise BtrfsScrubSchedulePolicyError(
            "Btrfs scrub schedule desired state has an unexpected schema"
        )
    if value["schema"] != DESIRED_SCHEMA or not isinstance(value["enabled"], bool):
        raise BtrfsScrubSchedulePolicyError("Btrfs scrub schedule desired state is invalid")
    return {"schemaVersion": SCHEMA_VERSION, "enabled": value["enabled"]}
...
def plan_policy(
    ...
    *,
    ...
    if desired["enabled"] and not installed:
        raise OSError("Btrfs scrub scheduler is not installed")
...
def _assert_parent(path: Path, *, trusted_uid: int) -> None:
    ...
    if not stat.S_ISDIR(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
        raise OSError("Btrfs scrub policy directory is unsafe")
    if metadata.st_uid != trusted_uid or (os.name == "posix" and metadata.st_mode & 0o022):
        raise OSError("Btrfs scrub policy directory has unsafe ownership or mode")
...
def apply_policy(
    ...
    *,
    ...
    if os.name == "posix" and os.geteuid() != trusted_uid:
        raise OSError("Btrfs scrub schedule update requires root")
    with _POLICY_LOCK:
        plan = plan_policy(
            desired_state,
            path=path,
        ...
        try:
            ...
            if verified != plan["desired"]:
                raise OSError("Btrfs scrub schedule verification failed")
            ...
            try:
                ...
                raise OSError(
                    "Btrfs scrub schedule update failed and rollback was incomplete"
                ) from rollback_exc
            raise OSError("Btrfs scrub schedule update failed and was rolled back") from exc
        return {**plan, "applied": True, "verified": True}


__all__ = [
    "BtrfsScrubSchedulePolicyError",
    "DESIRED_SCHEMA",
    "POLICY_PATH",
    "SCHEDULE",
    "TIMER_PATH",
