...
这是从飞牛 OS 逆向里搬过来的**最值得抄的一个手法**:飞牛的 nginx 有 30 个
``conf.d/*.conf`` 片段,每个功能模块一个独立的 unix socket,装一个新应用 =
放一个 conf 片段 + reload,没有注册中心、没有数据库、没有 migrations。

这里把同一套模式用在共享上:

- SMB:整个管控面只维护**一个** ``octopus.conf`` 片段,由共享清单全量重生成
  (幂等,不解析别人的 smb.conf);
- NFS:同理写一个 ``exports.d/octopus.exports``(nfs-kernel-server 原生支持
  ``exports.d/``);
...
class ShareManager:
    ...
    ``root`` 之下的数据目录布局::
        ...
        <root>/
          ...
          smb/octopus.conf     # 渲染产物
          exports.d/octopus.exports
    ...
    def render_smb_conf(self, shares: list[Share] | None = None) -> str:
        ...
        lines = [
            "# Generated by octopus-os. DO NOT EDIT — edit shares.json instead.",
            "# Atomic-rewritten; previous version kept as octopus.conf.bak",
            "",
        ]
        for s in smb:
            lines += [
    ...
    def apply(self) -> dict[str, Any]:
        ...
        shares = self._load()
        smb_path = self.root / "smb" / "octopus.conf"
        exports_path = self.root / "exports.d" / "octopus.exports"
