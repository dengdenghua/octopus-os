...
class BtrfsSnapshotRestoreCopyDesiredState(BaseModel):
    model_config = ConfigDict(extra="forbid", populate_by_name=True)

    schema_name: Literal["echo.omv.btrfs-snapshot-restore-copy-desired.v1"] = Field(
        default=BTRFS_SNAPSHOT_RESTORE_COPY_DESIRED_SCHEMA,
        alias="schema",
    )
    shared_folder_ref: str = Field(min_length=36, max_length=36, alias="sharedFolderRef")
    snapshot_id: str = Field(min_length=36, max_length=36, alias="snapshotId")
    ...
    @field_validator("shared_folder_ref", "snapshot_id", "name")
    @classmethod
    def validate_restore_copy_field(cls, value: str, info: Any) -> str:
        payload = {
            ...
            "name": value if info.field_name == "name" else "recovered_share",
        }
        try:
            normalized = validate_btrfs_snapshot_restore_copy_desired(payload)
        except ValueError as exc:
            raise ValueError(str(exc)) from exc
        key = {
            "shared_folder_ref": "sharedFolderRef",
...
class BtrfsSnapshotRestoreCopyApplyRequest(BaseModel):
    model_config = ConfigDict(extra="forbid", populate_by_name=True)

    desired: BtrfsSnapshotRestoreCopyDesiredState
    plan_id: str = Field(pattern=r"^[0-9a-f]{64}$", alias="planId")
