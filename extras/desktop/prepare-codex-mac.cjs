...
// macOS mirror of prepare-codex-win.cjs. Materializes the exact
// @openai/codex-darwin-x64 native package pinned by frontend's pnpm lock
// into the directory consumed by electron-builder. Build step only:
// released applications never contact npm or consult PATH.
//
// The darwin-x64 package is an optional dependency filtered out by pnpm on
// Apple Silicon hosts, so the script resolves it from node_modules when
// present and otherwise fetches the registry tarball pinned by the lockfile's
...
// ECHO_MAC_ARCH selects the target slice; it defaults to the host arch so
// native (arm64) builds need no extra flags and Rosetta/x64 stays opt-in.
...
const DARWIN_PROFILES = {
  arm64: {
    ...
    platformPackageVersion: `${CODEX_VERSION}-darwin-arm64`,
    // pnpm-lock.yaml: '@openai/codex@0.149.0-darwin-arm64' resolution.integrity
    integrity:
  ...
  x64: {
    ...
    platformPackageVersion: `${CODEX_VERSION}-darwin-x64`,
    // pnpm-lock.yaml: '@openai/codex@0.149.0-darwin-x64' resolution.integrity
    integrity:
      "sha512-H+mMgW3Nhc5QzGWEklCoFqACuOc0cVpgPkPQRw0LShoK7P5664T6BRnyl1yzT6orKPKv49cXry7DIWWZ19SanQ==",
    tarballUrl:
      "https://registry.npmjs.org/@openai/codex/-/codex-0.149.0-darwin-x64.tgz",
    targetTriple: "x86_64-apple-darwin",
  },
};
const PROFILE = DARWIN_PROFILES[MAC_ARCH];
if (!PROFILE) {
  throw new Error(
    `unsupported ECHO_MAC_ARCH: ${MAC_ARCH} (expected arm64 or x64)`,
  );
}
...
// Platform-neutral license texts reviewed for the Windows closure; the same
// rust crate versions ship in the darwin-x64 package, so the texts carry over.
const LICENSE_FILES = [
  {
    source: path.join(
      __dirname,
...
function verifyPinnedWrapper() {
  ...
  const lockedPlatformPackage = [
    `  '@openai/codex@${CODEX_VERSION}-darwin-${MAC_ARCH}':`,
...
function prepare() {
  ...
  for (const relative of requiredFiles) {
    ...
    if (
      ...
      header.toString("hex") !== "cffaedfe"
    ) {
      throw new Error(
        `copied Codex executable is not a Mach-O 64-bit binary: ${relative}`,
      );
    }
    ...
    `${JSON.stringify(
      {
        ...
        // macOS builds are not re-signed after this manifest is generated
        // (no Authenticode equivalent rewrites the binaries), but executable
        // hashes are still verified separately from license hashes at
        // runtime, mirroring the Windows flow.
