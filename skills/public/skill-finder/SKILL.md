---
name: skill-finder
description: Find, search, recommend, and install agent skills from the local skill catalog, skills.sh, ClawHub, and SkillsMP — all in one place. Always consult this when user asks to find, search, discover, or choose skills. Use when user says "how do I do X", "find a skill for X", "is there a skill that can...", "search all platforms", "best skill for X", "what skill should I use for X", "该用什么skill", "有没有XX的skill", "帮我找skill", "推荐一个skill", or expresses interest in extending capabilities or choosing the right skill for a task.
always_apply: true
---
...
|----------|----------|---------------|-------|-------------|------|
| **Local Skill Catalog** | Official/first-party skills, already vetted, instant install | Local file read | ⚡ Instant | ~60 | ❌ Not required |
...
- "from local catalog" / "built-in skills" / "our skills" → search Local Skill Catalog only
- "from skills.sh" → search skills.sh only
- "from clawhub" → search ClawHub only
- "from skillsmp" → search SkillsMP only

**Otherwise → ALWAYS start with the Local Skill Catalog (Step 1.5), then escalate if needed.**

### Step 1.5: Search Local Skill Catalog (ALWAYS do this first)

The local skill catalog contains all official/first-party skills — both installed and uninstalled. This is the fastest and most reliable source.
...
**After finding a match in the local skill catalog:**
...
1. Check if the skill is already in `<available_skills>` in the system prompt:
   ...
   - If NO → the skill needs to be installed. Tell the user you found a matching local skill and what it does, then offer to install it
...
**If a good local match is found → use it. Only proceed to external platforms if no suitable local skill exists.**

### Step 2: Choose Search Strategy (External Platforms)

Only reach this step if the Local Skill Catalog search in Step 1.5 found no suitable match.
...
| `clawhub install <slug> --version 1.2.3` | Install a specific version |
| `clawhub update <skill>` | Update a single skill |
| `clawhub update --all` | Update all skills |
| `clawhub update --all --no-input --force` | Force update all skills |
