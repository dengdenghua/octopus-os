...
<Link to={`/workspace/evolution?agent=${agentId}`}>
  <div className="rounded-lg border bg-card p-3 hover:bg-accent">
    ...
    <div className="mt-2">
      ...
      <div className="mt-1 text-xs text-muted-foreground">85% → Lv.24</div>
    </div>
    <div className="mt-2 text-xs text-primary">点击查看成长详情 →</div>
  </div>
</Link>
```
...
<header className="flex items-center justify-between border-b p-4">
  ...
  <div className="flex gap-2">
    ...
    <ToggleGroup type="single" value={viewMode} onValueChange={setViewMode}>
      <ToggleGroupItem value="gamified">🎮 游戏化</ToggleGroupItem>
