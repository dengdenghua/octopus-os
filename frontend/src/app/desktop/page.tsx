...
type DesktopApp = {
  ...
  color: string;
  // P2:agent 工作台类应用,原生路线开成桌面窗口(去 fork 后指向 /agent-ui/)。
  windowed?: boolean;
};
...
export default function DesktopShellPage() {
  ...
  const openApp = (app: DesktopApp) => {
    // P2:agent 工作台类应用原生路线开成桌面窗口(URL 由 resolveAgentAppUrl 决定:
    // 默认同源,投喂外部 webui 后指向 /agent-ui/);Electron 寄生模式整页打开。
    if (app.windowed && !isElectronShell) {
      openWindow({
