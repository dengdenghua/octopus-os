...
export function SmartTeamDialog({
  ...
  const candidates = useMemo<AgentWorldAgent[]>(() => {
    const localKeys = new Set(
      agents.flatMap((agent) => [agent.id, agent.name]),
    );
    const remote = cloudExperts
      .filter(
        (expert) => !localKeys.has(expert.id) && !localKeys.has(expert.name),
      )
      .map<AgentWorldAgent>((expert) => ({
        id: expert.id,
        name: expert.name || expert.id,
        display_name: expert.display_name || expert.name || expert.id,
        description: [expert.profession, expert.description]
          .filter(Boolean)
          .join(" · "),
  ...
  const plan = useMemo(
    () => buildSmartTeamPlan(task, candidates),
    [candidates, task],
  );
  ...
  const launch = async () => {
    ...
    try {
      ...
      const visibleLeadId =
        installedIds.get(visibleLead.id) || visibleLead.name;
      ...
      navigate(
        preserveWorkbenchPresentation(
          taskCollaboratorRouteForLeader(runtimeLead),
          search,
        ),
      );
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!task.trim() || plan.members.length === 0 || launching}
            onClick={() => void launch()}
          >
            {launching ? (
