import { quoteDatabaseFiles } from "@/core/composer-file-inbox";
...
import { downloadResourceUrl, resolveResourceLocation } from "./files";
import {
  isLocalDirectoryPickerUnavailableError,
  pickLocalDirectory,
} from "@/core/workspace/pick-local-directory";
...
function fileItemKey(item: { path: string; resourceId?: string }): string {
  return item.resourceId || `path:${item.path}`;
}
...
function isStorageConnectivityError(error: unknown): boolean {
  return isNASConnectivityError(error) || isBrowserNetworkError(error);
}

function storageErrorMessage(error: unknown, copy: StorageCopy): string {
  if (isNASAuthenticationError(error)) return copy.service.credentialsExpired;
  if (isStorageConnectivityError(error)) return copy.service.networkError;
...
function fileAssetToItem(asset: NASFileAsset): FileItem {
  ...
  return {
    ...
    resourceId: nasResourceId(asset),
    kind: extension,
...
export default function StoragePage() {
  ...
  const [policy, setPolicy] = useState<NASPolicy | null>(null);
  ...
  const refreshNAS = useCallback(async () => {
    try {
      ...
      setVideos([]);
      setAlbums([]);
      setServiceError(storageErrorMessage(error, copy));
      return false;
    }
  }, [copy]);
  ...
  const ensureNASService = useCallback(async () => {
    ...
    // A missing sibling or a failed launch cannot become ready by polling.
    // Return immediately so an offline Storage install does not create a burst
    // of identical 502 requests across every retained desktop/workbench host.
    if (startResult.status === "not_found") {
      setServiceError(copy.service.notFound);
      return false;
    }
    if (startResult.status === "error" || startResult.status === "disabled") {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        if (isStorageConnectivityError(error)) {
  ...
  const pickFolder = async () => {
    ...
    try {
      ...
      setServiceError(
        isLocalDirectoryPickerUnavailableError(err)
          ? copy.service.folderPickerUnavailable
          : storageErrorMessage(err, copy),
      );
    } finally {
  ...
  const startIndexing = async () => {
    ...
    try {
      ...
      const message = storageErrorMessage(error, copy);
  ...
  const togglePrivacy = async () => {
    ...
    setPolicyBusy(true);
    try {
      setPolicy(await updateNASPolicy({ ...policy, mode }));
    } catch (error) {
      toast.error(storageErrorMessage(error, copy));
    } finally {
      setPolicyBusy(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      void getNASPolicy().then(
        (next) => {
          if (mounted && requestId === latestRequest) setPolicy(next);
        },
        () => {
          if (mounted && requestId === latestRequest) setPolicy(null);
        },
      );
    };
    refresh();
        ...
        <div className="flex size-full overflow-hidden">
          <section className="workspace-panel flex min-h-0 flex-1 overflow-hidden rounded-none border-0 bg-card">
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden bg-card">
              {serviceError && activeLibrary !== "computer" && (
              ...
              {activeLibrary !== "sources" && (
                <div className="flex shrink-0 items-center justify-end gap-1.5 border-b border-border bg-muted px-3 py-2">
                  ...
                  <Button
                    ...
                    title={
                      policy?.mode === "privacy"
                        ? "切换为云端效率模式，任务内容和历史上下文可能发送至模型服务"
                        : "启用系统隐私模式，仅使用本机模型与本地工具"
                    }
                  >
                    {policy?.mode === "privacy" ? (
              ...
              {activeLibrary === "sources" ? (
                ...
                <LocalDiskView
                  ...
                  manifest={manifest}
                  browseOnlyMessage={
                    serviceError ? copy.service.browseOnly : undefined
                  }
                />
              ) : activeLibrary === "apps" ? (
                <AppsView
...
function VideoPlayerDialog({
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      ...
      >
        <DialogTitle className="sr-only">{target.video.name}</DialogTitle>
...
function QuickFileActions({
  ...
  compact?: boolean;
  item?: Pick<FileItem, "name" | "path" | "resourceId">;
  onPreview?: () => void;
}) {
  const { t } = useI18n();
  const copy = t.storage;
  ...
  const quoteInTask = () => {
    if (!item) return;
    quoteDatabaseFiles([item.path], null, [item.resourceId]);
  };
  const buttons = [
    {
...
function useFilePreview(item: FileItem | null): FilePreviewState {
  const kind = item ? previewKind(item) : null;
  const resourceId = item?.resourceId?.trim() || "";
  const hasPortableResource =
    resourceId.startsWith("storage-file:v1:") ||
    resourceId.startsWith("appliance-file:v1:");
  const textAssetId =
    item?.assetId || (hasPortableResource ? resourceId : undefined);
  const assetState = useNASAssetState(
    item?.assetId && kind && kind !== "text"
      ? `/v1/files/${encodeURIComponent(item.assetId)}/content`
      : resourceId.startsWith("storage-file:v1:") && kind && kind !== "text"
        ? `/v1/files/${encodeURIComponent(resourceId)}/content`
        : resourceId.startsWith("appliance-file:v1:") && kind && kind !== "text"
          ? downloadResourceUrl(resourceId)
          : undefined,
  );
  const [textState, setTextState] = useState<FilePreviewState>({
    loading: false,
    text: null,
    url: null,
    error: null,
  ...
  useEffect(() => {
    if (!textAssetId || kind !== "text") {
      setTextState({ loading: false, text: null, url: null, error: null });
      return;
    }

    let disposed = false;
    setTextState({ loading: true, text: null, url: null, error: null });
    const finish = (next: Omit<FilePreviewState, "loading">) => {
      if (!disposed) setTextState({ loading: false, ...next });
    };

    void loadNASAssetText(textAssetId)
      .then((text) => finish({ text, url: null, error: null }))
      .catch((error) =>
        finish({
          text: null,
          url: null,
...
function PreviewPanel({
  ...
  const copy = t.storage;
  const Icon = item.icon;
  const preview = useFilePreview(item);
  ...
  const [document, setDocument] = useState<NASTextDocument | null>(null);
  const [patch, setPatch] = useState("");
  const [editError, setEditError] = useState<string | null>(null);
  const operation = useRef(0);
  const canEdit =
    supportsNASTextEditing(manifest) &&
    Boolean(item.resourceId) &&
    kind === "text" &&
    preview.text !== null;
  const displayedText = savedText ?? preview.text;
  useEffect(() => {
    const dirty = saving || (editing && draft !== document?.text);
  ...
  }, [saving, editing, draft, document, onEditStateChange]);
  useEffect(() => {
    setEditing(false);
    setDraft("");
    setSavedText(null);
  ...
  }, [item.path, item.resourceId]);
  useEffect(() => {
    if (!editing && preview.text !== null && savedText === null) {
      setDraft(preview.text);
    }
  }, [editing, preview.text, savedText]);
  const quoteInTask = () => {
    quoteDatabaseFiles([item.path], null, [item.resourceId]);
  };
  ...
  const startEditing = async () => {
    ...
    setSaving(true);
    try {
      const full = await readNASTextDocument(item.resourceId);
      if (version !== operation.current) return;
      setDocument(full);
      setSavedText(full.text);
      setDraft(full.text);
      setEditError(null);
      setShowDiff(false);
      setEditing(true);
    } catch (error) {
      if (version === operation.current) toast.error(nasTextEditError(error));
    } finally {
      if (version === operation.current) setSaving(false);
    }
  };
  const cancelEditing = () => {
  ...
  const refreshBase = async () => {
    ...
    setSaving(true);
    try {
      const latest = await readNASTextDocument(item.resourceId);
      if (version !== operation.current) return;
      setDocument(latest);
      setSavedText(latest.text);
      setShowDiff(false);
      setEditError(null);
      toast.success("已读取最新版本，草稿已保留；请查看差异并合并后保存");
    } catch (error) {
      if (version === operation.current) setEditError(nasTextEditError(error));
    } finally {
      if (version === operation.current) setSaving(false);
    }
  };
  const toggleDiff = async () => {
    ...
    if (!document || !item.resourceId) return;
    const version = ++operation.current;
    setSaving(true);
    try {
      const result = await diffNASTextDocument(
        item.resourceId,
        draft,
        document.revision,
      );
      if (version !== operation.current) return;
      setPatch(result.patch || "没有修改");
      setShowDiff(true);
    } catch (error) {
      if (version === operation.current) setEditError(nasTextEditError(error));
    } finally {
  ...
  const saveText = async () => {
    ...
    setSaving(true);
    try {
      const saved = await saveNASTextDocument(
        item.resourceId,
        draft,
        document.revision,
      );
      ...
      <div className="mt-3 rounded-lg bg-card p-4 shadow-[var(--shadow-xs)] ring-1 ring-border">
        ...
        {preview.loading ? (
          ...
          editing ? (
            <div className="mt-2 space-y-2">
              {editError && (
                <div
                  role="alert"
                  className="rounded-lg border border-warning/30 p-3 text-xs"
                >
                  <p>{editError}</p>
                  <Button
                    size="sm"
              ...
              {showDiff ? (
                <pre
                  aria-label={`${item.name} 的修改差异`}
                  className="min-h-56 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-lg border border-border bg-muted/50 p-3 font-mono text-xs leading-5"
                >
                  {patch.split("\n").map((line, index) => (
                    <span
                      key={index}
                      className={cn(
                        "block",
                        line.startsWith("-") && "bg-red-500/10 text-red-700",
                        line.startsWith("+") &&
                          "bg-emerald-500/10 text-emerald-700",
                        !/^[+-]/.test(line) && "text-muted-foreground",
                      )}
                    >
                      {line || " "}
                    </span>
                ...
                <textarea
                  ...
                  className="min-h-56 w-full resize-y rounded-lg border border-border bg-muted/50 p-3 font-mono text-xs leading-5 outline-none focus-visible:ring-2 focus-visible:ring-ring"
                />
              )}
            ...
            <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-muted/50 p-3 font-mono text-xs leading-5 text-muted-foreground">
              {displayedText || "（文件为空）"}
            </pre>
          )
          ...
          <img
            src={preview.url}
            alt={item.name}
            className="mt-2 max-h-72 w-full rounded-lg bg-muted/50 object-contain"
          />
          ...
          <iframe
            title={`${item.name} 预览`}
            src={preview.url}
            className="mt-2 h-72 w-full rounded-lg border border-border bg-muted/50"
          />
        ) : (
...
export function LocalDiskView({
  ...
  const locatedFile = localFileLocation(locationParams.get("file") || "");
  const locatedResourceId = locationParams.get("resource_id")?.trim() || null;
  const locatedDirectory = locatedFile?.directory;
  const resourceOnlyId =
    !locatedFile && locatedResourceId?.startsWith("appliance-file:v1:")
      ? locatedResourceId
      : null;
  // The embedded desktop Storage fallback keeps its resource identity
  // portable. Decode its root-relative locator only when the manifest proves
  // this is the local provider; external Storage paths must never be treated
  // as local filesystem paths by the database view.
  const embeddedStorageLocation =
    !locatedFile &&
    manifest?.role === "embedded" &&
    locatedResourceId?.startsWith("storage-file:v1:")
      ? parseStorageFileResourceId(locatedResourceId)
      : undefined;
  const [resourceLocation, setResourceLocation] = useState<Awaited<
    ReturnType<typeof resolveResourceLocation>
  > | null>(null);
  const [resourceLocationError, setResourceLocationError] = useState<
    string | null
  >(null);
  useEffect(() => {
    if (!resourceOnlyId) {
      setResourceLocation(null);
      setResourceLocationError(null);
      return;
    }
    let cancelled = false;
    setResourceLocation(null);
    setResourceLocationError(null);
    void resolveResourceLocation(resourceOnlyId).then(
      (location) => {
        if (!cancelled) setResourceLocation(location);
      },
      (error: unknown) => {
        if (!cancelled)
          setResourceLocationError(
            error instanceof Error ? error.message : "无法解析文件位置",
          );
      },
    );
    return () => {
  ...
  const resourceDirectory =
    resourceLocation?.directory ||
    (embeddedStorageLocation
      ? embeddedStorageLocation.path.replace(/[\\/][^\\/]+$/, "") || "/"
      : null);
  const locationDirectory = locatedDirectory || resourceDirectory;
  const [currentPath, setCurrentPath] = useState(locationDirectory || "/");
  useEffect(() => {
    if (locationDirectory) setCurrentPath(locationDirectory);
  }, [locationDirectory]);
      ...
      try {
        ...
        setEntries(
          body.map((entry) => ({
            name: entry.name,
            path: entry.path,
            resourceId: nasResourceId(entry),
  ...
  const pathParts = directoryBreadcrumbs(currentPath);
  const goUp = () => {
    if (pathParts.length > 1)
      setCurrentPath(pathParts[pathParts.length - 2]!.path);
  };
    ...
    <>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        <main className="min-h-0 min-w-0 flex-1 overflow-hidden bg-card">
          <div className="grid grid-cols-[minmax(0,1fr)_120px_96px_112px] gap-3 border-b border-border bg-muted/30 px-3 py-2 text-xs font-medium text-muted-foreground">
            <span>名称</span>
            <span>类型</span>
            <span>项目</span>
            <span />
          ...
          <div className="min-h-0 overflow-y-auto">
            {!isLoading &&
              !browseError &&
              entries.length > 0 &&
              locationDirectory === currentPath &&
              !entries.some(
                (item) =>
                  (locatedResourceId &&
                    item.resourceId === locatedResourceId) ||
                  (locatedFile &&
                    normalizeLocalPath(item.path) === locatedFile.path),
              ) && (
                <p
                  role="status"
            ...
            {isLoading ? (
              ...
              entries.map((item) => (
                <LocalDiskEntryRow
                  ...
                  quoteLabel={
                    locationParams.get("sourceThread")
                      ? "引用到原任务"
                      : "引用到助手"
                  }
                  onQuote={
  ...
  return (
    <div className="relative flex items-center">
      <button
        ...
        ref={(node) => {
          if (located) node?.scrollIntoView?.({ block: "nearest" });
        }}
        className="grid w-full grid-cols-[minmax(0,1fr)_120px_96px_112px] items-center gap-3 border-b border-black/[0.035] px-3 py-2.5 text-left text-sm transition-colors hover:bg-muted aria-[current=true]:bg-accent"
      >
      ...
      {onQuote && (
        <button
          type="button"
          className="absolute right-3 rounded-md border px-2 py-1 text-xs hover:bg-muted"
          onClick={onQuote}
          aria-label={`${quoteLabel}：${item.name}`}
        >
          {quoteLabel}
        </button>
      )}
            ...
            {hits.map((hit, index) => (
              ...
              >
                <button
                  ...
                  aria-pressed={selectedPaths.has(nasResourceSelectionKey(hit))}
                  onClick={() =>
                    setSelectedPaths((current) => {
                      const next = new Set(current);
                      const selectionKey = nasResourceSelectionKey(hit);
          ...
          <div className="flex min-h-[340px] flex-col items-center justify-center rounded-lg bg-card px-6 text-center shadow-[var(--shadow-xs)] ring-1 ring-border">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
              <Button
                ...
                onClick={() => {
                  setLibraryParams((previous) => {
                    const next = new URLSearchParams(previous);
                    next.set("library", "sources");
                    return next;
                  });
                }}
              >
