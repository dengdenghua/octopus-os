"""Shared Echo-owned tools for execution engine adapters.

Engine adapters isolate ambient MCP/plugin/skill configuration. This module
is the narrow capability bridge back into the
...
from runtime.execution.tool_engine.effect_receipts import should_refresh_read_result
...
class HostToolRequest:
    """Transport data; authority is supplied only by the host constructor."""
...
class HostToolCatalog:
    """One immutable per-turn host tool advertisement."""
...
def dynamic_tool_failure(reason: Any) -> dict[str, Any]:
    """Return the bounded fail-closed result shape exposed to adapters."""
...
def _catalog_agent(agent: Any, policy: Any) -> Any:
    ...
    return SimpleNamespace(
        agent_id=str(getattr(agent, "agent_id", "") or "host-role"),
        arms=(
            SimpleNamespace(
                arm_id="host-effective-turn-policy",
...
class HostToolBroker:
    """Execute one frozen Echo tool catalog for one exact host turn."""
    ...
    def __init__(
        ...
        *,
        ...
        execution_engine: Literal["native", "codex", "opencode"],
        max_tools: int = _MAX_DYNAMIC_TOOLS,
    ) -> None:
        if execution_engine not in {"native", "codex", "opencode"}:
            raise ValueError("unsupported host execution engine")
        if type(max_tools) is not int or not 1 <= max_tools <= _MAX_DYNAMIC_TOOLS:
            raise ValueError("host tool catalog limit must be between 1 and 256")
        executor = getattr(stack, "executor", None)
        registry = getattr(executor, "registry", None)
        if executor is None or registry is None:
            raise ValueError("Host tools require the Echo execution stack")
        self._stack = stack
        self._agent = agent
        ...
        self._execution_lease = getattr(parent, "execution_lease", None)
        host_request = getattr(parent, "execution_request", None)
        self._execution_request: ExecutionRequest | None = None
        if isinstance(host_request, ExecutionRequest):
            task = host_request.task
            if task.thread_id != outer_thread_id or (
                task.actor_id is not None
                and (task.actor_id != principal_id or task.tenant_id != tenant_id)
            ):
                raise ValueError("Host tool scope does not match the host execution request")
            # The adapter supplies provenance; retain the original authority,
            # task identity and absolute deadline across callback workers.
            self._execution_request = replace(
                host_request, task=replace(task, execution_engine=execution_engine)
            )
        ...
        # Each callback gets an independent policy dictionary, but executor-
        # owned read evidence and write leases belong to the whole host turn.
        # Arbitrary engine/context keys cannot seed these containers.
        for key, factory in (
            ("_read_file_paths_this_turn", list),
            ("_file_write_leases", dict),
            ("_file_write_lease_handoffs", dict),
            ("_file_write_lease_history", list),
            ("_file_read_snapshots", dict),
        ):
            if not isinstance(self._metadata.get(key), factory):
    ...
    def bind_inner_scope(self, *, thread_id: str, turn_id: str) -> None:
        if not thread_id or not turn_id:
            raise ValueError("inner engine thread and turn ids must be non-empty")
        self._inner_thread_id = thread_id
        self._inner_turn_id = turn_id

    def skill_name(self, advertised_name: str) -> str | None:
        entry = self._entries.get(advertised_name)
        return entry.skill_name if entry else None

    def close(self) -> None:
        """Revoke admission immediately; aclose also drains admitted calls."""
        self._closed = True
    ...
    async def invoke(self, tool: str, arguments: dict[str, Any], *, call_id: str) -> dict[str, Any]:
        """Use transport coordinates bound by the adapter, never model input."""
        return await self(
            HostToolRequest(
                DYNAMIC_TOOL_CALL_METHOD,
                {
                    "threadId": self._inner_thread_id,
                    "turnId": self._inner_turn_id,
                    "callId": call_id,
                    "tool": tool,
                    "arguments": arguments,
                },
    ...
    async def __call__(self, request: HostToolRequest) -> dict[str, Any]:
        if self._interrupted():
            return dynamic_tool_failure("outer Echo turn was interrupted")
        # A disconnected transport cannot release the serialization lock while
        # its native worker is still writing, or abandon an unrecorded effect.
        task = asyncio.create_task(self._dispatch(request))
        self._pending.add(task)
        task.add_done_callback(self._settled)
        return await asyncio.shield(task)

    def _settled(self, task: asyncio.Task[dict[str, Any]]) -> None:
        self._pending.discard(task)
        if not task.cancelled():
            task.exception()  # Observe failure even if the transport disappeared.

    async def _dispatch(self, request: HostToolRequest) -> dict[str, Any]:
        async with self._lock:
            return await self._handle(request)

    async def _handle(self, request: HostToolRequest) -> dict[str, Any]:
        if request.method != DYNAMIC_TOOL_CALL_METHOD:
            return dynamic_tool_failure("unsupported host tool request")
        if self._interrupted():
            return dynamic_tool_failure("outer Echo turn was interrupted")
        ...
        if cached is not None:
            ...
            # A protected read must re-check its live authorization and ACL,
            # including on duplicate delivery. The policy is frozen in the
            # server catalog; model arguments cannot opt writes into refresh.
            if not should_refresh_read_result(entry.skill_ref):
        ...
        if action in {"ask", "confirm"} and not auto_approve:
            outer_request = EchoApprovalRequest(
                ...
                detail=(
                    f"{self._execution_engine} requested an Echo tool "
    ...
    def _request_approval(self, request: EchoApprovalRequest) -> Any:
        # Account-scoped review models must observe this host turn, not the
        # transport reader task's ambient Session (which may be unrelated).
        with session_scope(self._execution_session(auto_approve=False)):
            return self._approval_provider.request(request, timeout=120.0)
    ...
    def _interrupted(self) -> bool:
        try:
            return self._closed or bool(self._is_interrupted())
