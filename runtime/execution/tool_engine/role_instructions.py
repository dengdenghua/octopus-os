"""Shared Echo role, memory, mode and selected skill instructions.

Execution engines do not own a second persona registry. The functions
here translate an ordinary ``Agent`` (soul, memory, modes and Echo skills)
into server-owned engine inputs while keeping every path supplied by the
...
def compose_role_instructions(
    ...
    *,
    ...
    goal: str,
    registry: Any = None,
    max_chars: int = _MAX_DEVELOPER_INSTRUCTIONS,
) -> str:
    ...
    return _bounded(
        ...
        limit=min(max_chars, _MAX_DEVELOPER_INSTRUCTIONS),
    )
