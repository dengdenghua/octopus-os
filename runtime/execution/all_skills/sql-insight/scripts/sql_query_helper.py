            ...
            issues.append(
                {
                    "severity": "warning",
                    "rule": "deep-nesting",
                    "message": f"检测到 {subquery_count} 层嵌套子查询，可能难以优化且可读性差",
            ...
            issues.append(
                {
                    ...
                    "detail": f"全表扫描: {table_name}",
                    "suggestion": f"考虑为 {table_name} 的 WHERE/JOIN 条件列添加索引",
                }
            )
                ...
                issues.append(
                    {
                        ...
                        "detail": f"顺序扫描（全表扫描）: {table}，预估 {rows} 行",
                        "suggestion": f"为 {table} 的过滤条件列添加索引",
                    }
                )
            else:
                issues.append(
                    {
                        ...
                        "detail": f"顺序扫描: {table}，预估 {rows} 行（小表，可接受）",
                ...
                issues.append(
                    {
                        ...
                        "detail": f"行数估计偏差: 预估 {plan_rows} 行，实际 {actual_rows} 行",
