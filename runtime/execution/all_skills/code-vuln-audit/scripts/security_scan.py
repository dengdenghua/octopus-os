...
def scan_secrets(root, exclude_dirs, max_file_kb):
    ...
    for fpath, relpath in walk_files(root, exclude_dirs, max_file_kb):
        ...
        for line_num, line in enumerate(lines, start=1):
            stripped = line.strip()
            if (not stripped or stripped.startswith("#") or stripped.startswith("//")) and not any(
                kw in stripped.lower() for kw in ("key", "secret", "token", "password")
            ):
                continue
...
def format_text(target, modules, findings):
    ...
    lines.append(f"Time: {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    lines.append("")
...
def main():
    ...
    modules = []
    modules = ["deps", "secrets", "owasp"] if args.mode == "all" else [args.mode]
