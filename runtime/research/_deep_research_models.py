"""Pydantic models and type aliases for deep-research planning.

Extracted from ``deep_research.py`` as part of a structural split. These
contracts are imported by both the planner module and the helper module so
neither depends on the other for type definitions.
"""
...
class ResearchRouteDecision(BaseModel):
    ...
    schema_: str = Field(
        default="echo.subagent_route_decision.v1",
        alias="schema",
    )
    step_id: str | None = None
