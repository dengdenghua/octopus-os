...
def create_app(
    ...
    *,
    ...
    # 企业版插件(octopus-enterprise/docs/PM_INTERFACE.md)。与母体对齐删除,
    # 不再用 OCTOPUS_ENABLE_COMPANY flag。team_rooms/team_tasks(Agent 协调)
    # 保留;上面 team 事件广播经 getattr(app.state, "company_router", None) 因
    # company 不存在而自动跳过同步,无需改动。
