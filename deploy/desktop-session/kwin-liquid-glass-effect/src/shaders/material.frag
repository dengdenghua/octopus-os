...
{
    vec4 sum = texture2D(texUnit, sampleUv + vec2(-halfpixel.x * 2.0, 0.0) * offset);
    sum += texture2D(texUnit, sampleUv + vec2(-halfpixel.x, halfpixel.y) * offset) * 2.0;
    sum += texture2D(texUnit, sampleUv + vec2(0.0, halfpixel.y * 2.0) * offset);
    sum += texture2D(texUnit, sampleUv + vec2(halfpixel.x, halfpixel.y) * offset) * 2.0;
    sum += texture2D(texUnit, sampleUv + vec2(halfpixel.x * 2.0, 0.0) * offset);
    sum += texture2D(texUnit, sampleUv + vec2(halfpixel.x, -halfpixel.y) * offset) * 2.0;
    sum += texture2D(texUnit, sampleUv + vec2(0.0, -halfpixel.y * 2.0) * offset);
    sum += texture2D(texUnit, sampleUv + vec2(-halfpixel.x, -halfpixel.y) * offset) * 2.0;
    return sum / 12.0;
...
void main(void)
{
    ...
    float red = texture2D(texUnit, clamp(refractedUv + chroma, vec2(0.001), vec2(0.999))).r;
    float blue = texture2D(texUnit, clamp(refractedUv - chroma, vec2(0.001), vec2(0.999))).b;
